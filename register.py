...
#from https://www.jianshu.com/p/a8c1b45dc0c0

class PunchBack:

    def __init__(self):
        self.phone = input('请输入您要轰炸的号码：')
        self.name = input('请输入您给他起的名字：')
        self.password = input('请输入您给他设置的密码：')
        self.num = 0
    ...
    def main(self):
        while True:            
            #self.qq('qq')                           #qq有点厉害 不知道为什么就是自动点击不认可        
            self.guazi('瓜子')
            self.wphui('唯品会')
            self.suning('苏宁')
            #self.yhd('一号店')                      #一号店也要识别
            self.youzan('有赞')
            self.pinduoduo('拼多多')
            self.dianping('大众点评')
            self.tangeche('弹个车')
